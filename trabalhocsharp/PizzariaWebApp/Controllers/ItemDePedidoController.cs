...
namespace PizzariaWebApp.Controllers
{
    ...
    public class ItemDePedidoController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult<ItemDoPedido>> AddItemDoPedido(ItemDoPedido itemdopedido)
        {
            await _itemdopedidoRepository.Add(itemdopedido); // Corrigido 'pedido' para 'itemdopedido'
            return CreatedAtAction(nameof(GetItemDoPedidoById), new { id = itemdopedido.Id }, itemdopedido); // Corrigido 'GetPedidoById' para 'GetItemDoPedidoById'
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateItemDoPedido(int id, ItemDoPedido itemdopedido)
        {
